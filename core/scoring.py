...
def calculate_environmental_score(temporal_score, has_da_path, controlled_object_count, shared_with,
                                  domain_risk_level=None, hibp_breach_count=0):
    """
    ...
    Args:
        ...
        hibp_breach_count (int, optional): HIBP breach count for this password/hash

    Returns:
        tuple: (environmental_score, privilege_factor, share_factor, domain_factor, hibp_factor)
    """
    ...
    # HIBP breach factor (1.0-1.5)
    # Passwords/hashes found in breaches are inherently more risky
    hibp_factor = 1.0
    if hibp_breach_count > 0:
        if hibp_breach_count >= 100000:      # Extremely common breach
            hibp_factor = 1.5
        elif hibp_breach_count >= 10000:     # Very common breach
            hibp_factor = 1.4
        elif hibp_breach_count >= 1000:      # Common breach
            hibp_factor = 1.3
        elif hibp_breach_count >= 100:       # Moderately common breach
            hibp_factor = 1.2
        else:                                # Rare breach
            hibp_factor = 1.1
    ...
    return min(10.0, environmental_score), privilege_factor, share_factor, domain_factor, hibp_factor
...
def calculate_password_risk_score(password_analysis, shared_with, da_domains, controlled_object_count,
    ...
    # CRACKED PASSWORD MINIMUM RISK
    # Evidence-based risk tiers derived from HIBP distribution analysis
    # (1.3B total hashes analyzed, max count: 132M occurrences)
    # This ensures cracked passwords have baseline risk regardless of apparent strength,
    # with differentiation based on actual exposure prevalence.

    # Tier 1: Ultra-extreme exposure (top 0.00001% - only 123 hashes worldwide)
    # These are THE most common passwords globally (e.g., blank password, "123456")
    if hibp_breach_count >= 1000000:
        base_score = max(base_score, 8.0)

    # Tier 2: Extreme exposure (top 0.0003% - ~3,257 hashes)
    # Extremely common passwords found in nearly every breach
    elif hibp_breach_count >= 100000:
        base_score = max(base_score, 7.5)

    # Tier 3: Very high exposure (top 0.01% - ~128K hashes)
    # OR flagged as common password in wordlists
    elif hibp_breach_count >= 10000 or is_common:
        base_score = max(base_score, 7.0)

    # Tier 4: High exposure (top 0.24% - ~3.2M hashes)
    # OR exact dictionary word
    elif hibp_breach_count >= 1000 or is_dictionary_word:
        base_score = max(base_score, 6.0)

    # Tier 5: Medium-high exposure (top 3% - ~39M hashes)
    # Moderately common across breaches
    elif hibp_breach_count >= 100:
        base_score = max(base_score, 5.0)

    # Tier 6: Medium exposure (top 20% - ~229M hashes)
    # OR contains identifiable patterns
    elif hibp_breach_count >= 10 or banned_words_count > 0 or keyboard_patterns_count > 0:
        base_score = max(base_score, 4.0)

    # Tier 7: Low exposure (any HIBP presence or short password)
    # Found in at least one breach, or length below NIST recommendations
    elif hibp_breach_count > 0 or password_length < 12:
        base_score = max(base_score, 3.0)

    # Tier 8: Minimal risk (long complex password, not in HIBP)
    # Difficult offline crack - strong password that was still cracked
    else:
    ...
    final_score, privilege_factor, share_factor, domain_factor, hibp_factor = calculate_environmental_score(
        temporal_score, has_da_path, controlled_object_count, shared_with, domain_risk_level, hibp_breach_count
    )
    ...
    score_breakdown = {
        ...
        "environmental_components": {
            ...
            "hibp_factor": round(hibp_factor, 2)
        }
    }
